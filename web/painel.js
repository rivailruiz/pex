...
function projectMetrics() {
  ...
  const saldoAtual = stockRows.reduce((sum, row) => sum + row.stock, 0);
  const estoqueBaixo = stockRows.filter((row) => row.stock <= state.lowStockThreshold).length;

  return {
    totalEntradas,
    totalSaidas,
    saldoAtual,
    itensAtivos: stockRows.length,
    estoqueBaixo
  };
}
...
function markdownTable(headers, rows) {
  const head = `| ${headers.map(sanitizeMdCell).join(" | ")} |`;
  const sep = `| ${headers.map(() => "---").join(" | ")} |`;
  const body = rows.length
    ? rows.map((row) => `| ${row.map(sanitizeMdCell).join(" | ")} |`).join("\n")
    : `| ${headers.map(() => "-").join(" | ")} |`;
  return [head, sep, body].join("\n");
}

function mdField(value, fallback) {
  const clean = safeText(value);
  return clean || `[Preencher] ${fallback}`;
}

function buildProjectReportMarkdown() {
  ...
  const stockRows = currentStockRows().map((row) => [
    row.name,
    row.category,
  ...
  return [
    `# Projeto de Extensao V (ADS) - ${projectName}`,
    "",
    "## 1. Identificacao geral",
    `- Data de geracao: ${generatedAt}`,
    "- Curso: Analise e Desenvolvimento de Sistemas (ADS)",
    "- Etapa: PEX V - Construcao e Execucao Preliminar do Projeto",
    `- Projeto: ${projectName}`,
    `- Instituicao parceira: ${partner}`,
    `- Objetivo (ODS): ${objective}`,
    "",
    "## 2. Documento de Estruturacao Completa do Projeto",
    "",
    "### 2.1 Introducao e justificativa",
    `- Contexto: ${mdField(
      planning.introContext,
      "Descrever instituicao/comunidade, contato inicial e problemas/oportunidades identificados."
    )}`,
    `- Demandas/Lacunas tecnologicas: ${mdField(
      planning.introGaps,
      "Explicar por que a solucao ADS e necessaria (software sob medida, integracao, dados etc.)."
    )}`,
    `- Relevancia: ${mdField(
      planning.introRelevance,
      "Apresentar importancia social, tecnologica e economica da intervencao."
    )}`,
    "",
    "### 2.2 Objetivos",
    `- Objetivo geral: ${mdField(planning.objectiveGeneral, "Informar a meta principal do projeto.")}`,
    `- Objetivos especificos: ${mdField(
      planning.objectiveSpecific,
      "Listar metas tangiveis (requisitos, prototipo, testes, treinamento)."
    )}`,
    "",
    "### 2.3 Publico-alvo e comunidade envolvida",
    `- ${mdField(
      planning.targetAudience,
      "Caracterizar beneficiarios e nivel de conhecimento tecnologico."
    )}`,
    "",
    "### 2.4 Metodologia e plano de acao",
    `- Coleta de requisitos: ${mdField(
      planning.requirementsCollection,
      "Descrever entrevistas, questionarios, brainstorming e analise de fluxos."
    )}`,
    `- Modelagem e desenvolvimento: ${mdField(
      planning.modelingDevelopment,
      "Detalhar UML, arquitetura, linguagens e padroes de projeto."
    )}`,
    `- Testes e validacao: ${mdField(
      planning.testsValidation,
      "Informar estrategia de testes funcionais, usabilidade e seguranca."
    )}`,
    `- Treinamentos: ${mdField(
      planning.trainingPlan,
      "Descrever capacitacao prevista para usuarios/operacao/manutencao."
    )}`,
    `- Documentacao: ${mdField(
      planning.documentationPlan,
      "Registrar manuais, diagramas, atas, logs e padrao de evidencias."
    )}`,
    "",
    "### 2.5 Recursos necessarios",
    `- Materiais/tecnicos: ${mdField(
      planning.resourcesMaterials,
      "Equipamentos, servidores, IDE, software e bancos de dados."
    )}`,
    `- Humanos: ${mdField(planning.resourcesHuman, "Equipe envolvida e estimativa de horas.")}`,
    `- Orcamentarios: ${mdField(
      planning.resourcesBudget,
      "Custos com infraestrutura, licencas, deslocamento etc."
    )}`,
    "",
    "### 2.6 Cronograma, responsaveis e marcos",
    `- Cronograma: ${mdField(planning.schedule, "Definir fases, datas e entregas intermediarias.")}`,
    `- Responsaveis/milestones: ${mdField(
      planning.milestonesResponsibles,
      "Atribuir equipe e pontos de verificacao."
    )}`,
    "",
    "### 2.7 Indicadores e avaliacao",
    `- Indicadores quantitativos: ${mdField(
      planning.indicatorsQuant,
      "Funcionalidades, desempenho, reducao de retrabalho, satisfacao numerica."
    )}`,
    `- Indicadores qualitativos: ${mdField(
      planning.indicatorsQual,
      "Usabilidade percebida, adequacao e confianca no sistema."
    )}`,
    `- Periodicidade de avaliacao: ${mdField(
      planning.indicatorsPeriodicity,
      "Revisoes semanais/quinzenais/mensais e como os dados serao coletados."
    )}`,
    "",
    "## 3. Relatorio de Execucao Preliminar (Piloto)",
    `- Selecao e planejamento do piloto: ${mdField(
      pilot.pilotSelectionPlanning,
      "Informar funcionalidade-chave e ambiente de homologacao."
    )}`,
    `- Implementacao da versao reduzida: ${mdField(
      pilot.pilotImplementation,
      "Descrever deploy/configuracao, cenarios e grupo de teste."
    )}`,
    `- Evidencias do piloto: ${mdField(
      pilot.pilotEvidence,
      "Listar prints, logs, questionarios e depoimentos coletados."
    )}`,
    `- Erros e ajustes: ${mdField(
      pilot.pilotErrorsAdjustments,
      "Registrar bugs, falhas e backlog de correcoes."
    )}`,
    `- Conclusoes da execucao preliminar: ${mdField(
      pilot.pilotExecutionReport,
      "Sintetizar resultados, licoes aprendidas e viabilidade."
    )}`,
    "",
    "## 4. Revisao e refinamento do projeto",
    `- Revisao do projeto: ${mdField(
      pilot.projectReview,
      "Descrever ajustes de objetivo, cronograma e planejamento."
    )}`,
    `- Evidencias opcionais de palestras/treinamentos: ${mdField(
      pilot.optionalTraining,
      "Detalhar tema, formato e publico atendido (opcional)."
    )}`,
    `- Registros dos treinamentos: ${mdField(
      pilot.optionalTrainingEvidence,
      "Informar materiais, fotos, listas de presenca e feedbacks (opcional)."
    )}`,
    `- Refinamento final e continuidade: ${mdField(
      pilot.finalRefinement,
      "Apresentar versao atualizada do plano e proximos passos."
    )}`,
    "",
    "## 5. Resultados quantitativos do painel",
    `- Entradas acumuladas: ${formatQty(metrics.totalEntradas)}`,
    `- Saidas acumuladas: ${formatQty(metrics.totalSaidas)}`,
    `- Saldo atual: ${formatQty(metrics.saldoAtual)}`,
    `- Itens em estoque: ${metrics.itensAtivos}`,
    `- Itens com estoque baixo (<= ${formatQty(state.lowStockThreshold)}): ${metrics.estoqueBaixo}`,
    "",
    "## 6. Estoque atual",
    markdownTable(
      ["Item", "Categoria", "Quantidade", "Ultima movimentacao"],
      stockRows
    ),
    "",
    "## 7. Movimentacoes recentes (ultimas 20)",
    markdownTable(
      ["Data", "Tipo", "Item", "Qtd", "Origem/Destino", "Responsavel", "Obs"],
      movementRows
    ),
    "",
    "## 8. Checklist de evidencias para entrega",
    "- [ ] Termo de autorizacao assinado",
    "- [ ] Fotos da execucao presencial",
    "- [ ] Prints do sistema/painel em uso",
    "- [ ] Registros de testes e feedback dos usuarios",
    "- [ ] Documento de estruturacao completa (ADS V)",
    "- [ ] Relatorio de execucao preliminar (piloto)",
    "- [ ] Versao refinada do plano de acao",
    "",
    "## 9. Autoavaliacao",
    "- [Preencher] Aprendizados tecnicos e de gestao obtidos no projeto.",
    "- [Preencher] Desafios encontrados e como foram tratados.",
    "- [Preencher] Melhorias propostas para a proxima fase do PEX.",
    "",
    `> Documento gerado automaticamente em ${reportDate} para apoio a entrega do PEX V (ADS).`
  ].join("\n");
}

function exportProjectReport() {
  const markdown = buildProjectReportMarkdown();
  const blob = new Blob([markdown], { type: "text/markdown;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  const fileDate = todayISO();
  const baseName = (safeText(state.project.name) || "relatorio-pex")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 60);
  a.href = url;
  a.download = `${baseName || "relatorio-pex"}-${fileDate}.md`;
  a.click();
...
function bindEvents() {
  ...
  els.exportReport.addEventListener("click", exportProjectReport);
